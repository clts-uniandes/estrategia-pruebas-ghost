...
When("I select post tag {string}", async function (tag) {
  const elements = new PostEditorPage(this.driver);
  const validateString = new utilsFaker(tag);
  const randomValue = validateString.validateTypeParam;
...
When("I can't click on the publish link", async function () {
  const pageEditorPage = new PostEditorPage(this.driver);
  const publishBtn = pageEditorPage.eleUpdateLink;
  return await assert(publishBtn.hasOwnProperty("error"))
});
Then("", async function () {
  const postEditorPage = new PostEditorPage(this.driver);
...
Then("I should see error {string}", async function (text) {
  let element = await this.driver.$(`//div[@class='gh-alert-content'][contains(text(), '${text}')]`);
