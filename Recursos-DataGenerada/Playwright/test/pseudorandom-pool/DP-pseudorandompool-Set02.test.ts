...
let testCode: string =  'DP003';
let testNumber:number = 0;
let i:number=1;

let randomElement: RandomElement = new RandomElement();
let pseudoRandomData: PseudoRandomData = new PseudoRandomData();
let dataPageList = pseudoRandomData.getPageDataListWithTitleAndContentAndPageUrlAndExcerptAndMetadataValues();
...
for (const pageData of dataPageList) {
    test.describe(`${testCode} - Page Feature`, () => {
        ...
        test(`${i++} - should create a page with random title and random content after that update page with random preload data`, async () => {
            ...
            //META DATA
            await pageEditor.clickMetaDataSection();
            await page.screenshot({path: utilities.generateScreenshotPath(screenshotNumber++)});
            await pageEditor.fillMetaTitle(pageData.get(PageFields.META_DATA_TITLE));
            await pageEditor.fillMetaDescription(pageData.get(PageFields.META_DATA_DESCRIPTION));
            await pageEditor.fillMetaCanonicalUrl(pageData.get(PageFields.META_DATA_CANONICAL_URL));
            await page.screenshot({path: utilities.generateScreenshotPath(screenshotNumber++)});
            await pageEditor.closeMetaDataSection();
            await page.screenshot({path: utilities.generateScreenshotPath(screenshotNumber++)});
