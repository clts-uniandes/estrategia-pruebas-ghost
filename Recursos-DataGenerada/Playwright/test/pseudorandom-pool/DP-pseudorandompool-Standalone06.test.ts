...
test.describe("PDPRX16 - Actualizacion perfil de usuario, todos los valores bajo el límite pero correo no sigue formato, \
        nuevo post author, el mail es el de siempre", () => {
    ...
    test.beforeAll( async() => {
        ...
        randomTrialEmail = randomElement.useFaker(FakerCategories.ALPHANUMERIC, 20);
    ...
    test("P: Pseudorandom (pool) dynamic, R: Robustez, not an email", async () => {
        ...
        //Then email remains the same
        await home.clickUserMenu();
        await home.clickUserProfileLink();
        const currentEmail = await staffEditorPage.eleEmailInputContent;
        expect(currentEmail).toContain(Env.USER);
        await new Promise(r => setTimeout(r, 2000));
    });
